...
- **Decision:** Add a `price_bc` column to the master DataFrame exported by `scripts/data_processor.py` (computed via `sklearn.preprocessing.PowerTransformer(method='box-cox')`) and include it in `train.parquet` and `test.parquet`. Also persist the fitted transformer object alongside the exported parquets.
- **Reasoning:** Box–Cox (fitted via `PowerTransformer`) finds a power transform that can better normalize skewed, strictly-positive data. Persisting the transformer ensures reproducible inverse transforms for reporting RMSE in dollars.

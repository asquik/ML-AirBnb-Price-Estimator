...
- **Context:** A two-way train-test split is insufficient for proper model development. Standard ML practice requires three splits: train (fit model + tune hyperparameters), validation (select best hyperparameters), and test (final untouched evaluation).
...
- **Reasoning:**
  ...
  - **True generalization estimate:** Test set remains pristine until final evaluation, giving an honest estimate of model performance on unseen data.
...
- **Impacted files:** 
  ...
  - `README.md` (update quickstart and data distribution notes)
...
- **Context:** For fair model comparison and reproducibility, all models must evaluate on the same held-out test set. Previously, data processor only loaded/cleaned data; train/test splitting was deferred to individual model scripts (inconsistent).
...
- **Reasoning:** 
  ...
  - Single source of truth: parquets shared across machines; all scripts load pre-split data → no re-splitting bugs.
  - Lightweight: train.parquet (20,973 rows) + test.parquet (5,243 rows) can be quickly downloaded for model training.
- **Impacted files:** `scripts/data_processor.py` (added `split_and_export()` method), `tests/test_data_processor.py` (added 3 new unit tests).
...
## 2026-03-20 — Finalize experiment workflow & development roadmap
- **Context:** Need a clear roadmap from local experiments to final model release, emphasizing Decision Tree as first model.
- **Decision:** Established a phased development roadmap: Decision Tree baseline (Phase 1), text branch (Phase 2), image branch (Phase 3), fusion (Phase 4), packaging & deployment (Phase 5).
- **Reasoning:** Structured iteration keeps experiments organized and results logged. Decision Tree goes first (simplicity and interpretability as a baseline). Each branch is optional, allowing flexible scope. Final documentation is a clean narrative: problem → baseline → text branch → image branch → comparison → conclusions.
- **Impacted files:** `README.md` (updated), `decision_log.md`.
- **Validation:** Roadmap includes clear deliverables, decision log integration points, and release checklist.
- **Next action:** Implement Decision Tree regression baseline trainer (Phase 1).

"""
FusionMLP training script for the Multi-Modal Airbnb Price Predictor.

Model registry coverage:
    #27  fusion_mlp.py  FusionMLP  normal_bc   shallow_64  224
    #28  fusion_mlp.py  FusionMLP  cleaned_bc  shallow_64  224
    #29  fusion_mlp.py  FusionMLP  normal_bc   deep_256    224   (head ablation)
    #30  fusion_mlp.py  FusionMLP  normal_bc   shallow_64  336   (resolution ablation; falls back to 224 if 336 embeddings are unavailable)

The script consumes pre-computed DistilBERT + CLIP embeddings and concatenates
...
from training_utils import (
    CATEGORICAL_COLS, NUMERIC_COLS, TABULAR_BASE_COLS as FEATURE_COLS,
...
class FusionDataset(Dataset):
    """Fully in-memory dataset for tabular + text + image fusion training."""
    ...
    def __init__(self, bundle: SplitBundle) -> None:
        ...
        self.image_embeddings = bundle.image_embeddings.astype(np.float32, copy=False)
    ...
    def __getitem__(self, idx: int):
        return (
            ...
            torch.from_numpy(self.image_embeddings[idx]),
...
class FusionMLP(nn.Module):
    """Late-fusion MLP over precomputed text+image embeddings and tabular features."""
    ...
    def __init__(
        ...
        prev_dim = text_dim + image_dim + len(NUMERIC_COLS) + sum(embedding_dims)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train FusionMLP on precomputed text+image embeddings")
    parser.add_argument(
        "--variant",
        required=True,
    ...
    parser.add_argument(
        ...
        choices=[224, 336],
        default=224,
        help="Requested CLIP embedding resolution. Falls back to 224 if 336 files are unavailable.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
...
def _find_text_embedding_matrix(split: str, variant: str) -> Path:
    ...
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise FileNotFoundError(
        f"Could not find text embedding matrix for split='{split}' variant='{variant}'. "
        f"Looked for: {[path.name for path in candidates]}"
    )


def _find_image_embedding_matrix(split: str, variant: str, requested_size: int) -> tuple[Path, int]:
    suffix = "_cleaned" if variant.startswith("cleaned") else "_normal"
    preferred = [requested_size]
    if requested_size == 336:
        preferred.append(224)

    for size in preferred:
        candidate = EMBED_DIR / f"{split}_image{suffix}_{size}.npy"
        if candidate.exists():
            return candidate, size

    candidates = [EMBED_DIR / f"{split}_image{suffix}_{size}.npy" for size in (224, 336)]
    raise FileNotFoundError(
        f"Could not find image embedding matrix for split='{split}' variant='{variant}' image_size={requested_size}. "
        f"Looked for: {[path.name for path in candidates]}"
    )


def load_text_embedding_split(split: str, variant: str) -> tuple[np.ndarray, np.ndarray]:
    matrix_path = _find_text_embedding_matrix(split, variant)
...
def load_image_embedding_split(split: str, variant: str, requested_size: int) -> tuple[np.ndarray, np.ndarray, int]:
    matrix_path, actual_size = _find_image_embedding_matrix(split, variant, requested_size)
    embeddings = np.load(matrix_path, mmap_mode=None, allow_pickle=False).astype(np.float32, copy=False)
    ids = _load_ids_file(matrix_path)
    return embeddings, ids, actual_size
...
def maybe_smoke_slice(bundle: SplitBundle, max_rows: int = 100) -> SplitBundle:
    return SplitBundle(
        ...
        image_embeddings=bundle.image_embeddings[:max_rows].copy(),
...
def load_split_bundle(split: str, variant: str, target_col: str, requested_size: int) -> tuple[SplitBundle, int]:
    df = load_tabular_split(split, variant)
    text_embeddings, text_ids = load_text_embedding_split(split, variant)
    image_embeddings, image_ids, actual_image_size = load_image_embedding_split(split, variant, requested_size)

    tab_ids = df["listing_id"].to_numpy(dtype=str)
    assert np.array_equal(
        tab_ids, text_ids
    ), "ALIGNMENT FAILURE: tabular and text embedding row order do not match"
    assert np.array_equal(
        tab_ids, image_ids
    ), "ALIGNMENT FAILURE: tabular and image embedding row order do not match"
...
def evaluate_split(
    ...
    with torch.no_grad():
        for text_embeddings, image_embeddings, categorical_features, numeric_features, _, y_raw, _ in loader:
            ...
            preds = model(text_embeddings, image_embeddings, categorical_features, numeric_features).detach().cpu().numpy()
...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for text_embeddings, image_embeddings, categorical_features, numeric_features, targets, _, sample_weight in train_loader:
            text_embeddings = text_embeddings.to(device, non_blocking=True)
            image_embeddings = image_embeddings.to(device, non_blocking=True)
            categorical_features = categorical_features.to(device, non_blocking=True)
            numeric_features = numeric_features.to(device, non_blocking=True)
            ...
            preds = model(text_embeddings, image_embeddings, categorical_features, numeric_features)
        ...
        val_count = 0
        with torch.no_grad():
            for text_embeddings, image_embeddings, categorical_features, numeric_features, targets, _, sample_weight in val_loader:
                text_embeddings = text_embeddings.to(device, non_blocking=True)
                image_embeddings = image_embeddings.to(device, non_blocking=True)
                categorical_features = categorical_features.to(device, non_blocking=True)
                numeric_features = numeric_features.to(device, non_blocking=True)
...
def main() -> None:
    ...
    print("=" * 80)
    print(f"FusionMLP | variant={variant} | target={target_col} | head={args.fusion_head} | image_size={args.image_size}")
    ...
    train_bundle, train_image_size = load_split_bundle("train", variant, target_col, args.image_size)
    val_bundle, val_image_size = load_split_bundle("val", variant, target_col, args.image_size)
    if train_image_size != val_image_size:
        raise RuntimeError(
            f"Image size mismatch between train ({train_image_size}) and val ({val_image_size}) embeddings."
        )

    actual_image_size = train_image_size
    if args.image_size == 336 and actual_image_size != 336:
        print("[WARN] 336px embeddings were unavailable; falling back to 224px embeddings.")
    ...
    if price_transformer is not None:
        ...
        print(f"Loaded price transformer lambda={box_cox_lambda:.6f}")
    ...
    tracker = ExperimentTracker(
        ...
        config={
            ...
            "image_embedding_source": "precomputed_clip",
            "requested_image_size": int(args.image_size),
            "actual_image_size": int(actual_image_size),
            "text_embedding_dim": int(train_bundle.text_embeddings.shape[1]),
            "image_embedding_dim": int(train_bundle.image_embeddings.shape[1]),
    ...
    train_dataset = FusionDataset(train_bundle)
    val_dataset = FusionDataset(val_bundle)
    ...
    model = build_model_from_cardinalities(
        ...
        image_dim=int(train_bundle.image_embeddings.shape[1]),
    ...
    test_bundle, test_image_size = load_split_bundle("test", variant, target_col, args.image_size)
    if args.smoke_test:
        print("[SMOKE TEST] Truncating test split to 100 rows immediately after load.")
        test_bundle = maybe_smoke_slice(test_bundle, 100)

    if test_image_size != actual_image_size:
        raise RuntimeError(
            f"Image size mismatch between train/val ({actual_image_size}) and test ({test_image_size}) embeddings."
        )

    test_dataset = FusionDataset(test_bundle)

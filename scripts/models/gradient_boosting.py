"""
GradientBoosting model — runs #7-8 in the model registry.

Usage:
    python scripts/models/gradient_boosting.py --variant normal_raw
    python scripts/models/gradient_boosting.py --variant cleaned_raw
    python scripts/models/gradient_boosting.py --variant normal_raw --smoke-test
"""
...
def main() -> None:
    ...
    print(f"\n{'='*70}")
    print(f"GradientBoosting  |  variant={variant}  |  target={target_col}")
    print(f"{'='*70}\n")
    ...
    sweep_depth = [3]          if args.smoke_test else MAX_DEPTHS
    sweep_lr    = [0.1]        if args.smoke_test else LEARNING_RATES
    loss_fn     = "huber" if variant == "normal_raw" else "squared_error"

    tracker = ExperimentTracker(
        model_type="GradientBoosting", modalities="tabular", variant=variant,
        run_name=args.run_name, is_smoke_test=args.smoke_test,
        config={"n_estimators_searched": sweep_n_est, "max_depths_searched": sweep_depth,
                "learning_rates_searched": sweep_lr, "loss": loss_fn,
    ...
    total = len(sweep_n_est) * len(sweep_depth) * len(sweep_lr)
    print(f"\nSweeping {total} configurations on val set...")
    print(f"{'n_est':<8} {'depth':<8} {'lr':<8} {'Val RMSE $':<14} {'Val MAE $':<12} {'Val R²':<8}")
    print("-" * 62)
        ...
        for depth in sweep_depth:
            for lr in sweep_lr:
                gb = GradientBoostingRegressor(
                    ...
                    loss=loss_fn, random_state=42, subsample=0.8,
                )
                gb.fit(X_train, y_train, sample_weight=sw_train)
                val_pred_raw = to_raw_dollars(gb.predict(X_val), price_transformer)
                vm = compute_metrics(y_val_raw, val_pred_raw)
                print(f"{n_est:<8} {depth:<8} {lr:<8} {vm['rmse']:<14.2f} {vm['mae']:<12.2f} {vm['r2']:<8.4f}")

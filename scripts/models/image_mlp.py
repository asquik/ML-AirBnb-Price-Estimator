"""
ImageMLP training script for the Multi-Modal Airbnb Price Predictor.

Model registry coverage:
    #23  image_mlp.py  ImageMLP  normal_bc   shallow_64  224
    #24  image_mlp.py  ImageMLP  cleaned_bc  shallow_64  224
    #25  image_mlp.py  ImageMLP  normal_bc   deep_256    224   (head ablation)
    #26  image_mlp.py  ImageMLP  normal_bc   shallow_64  336   (resolution ablation; falls back to 224 if 336 embeddings are unavailable)

The script consumes pre-computed CLIP image embeddings from disk and concatenates
...
class ImageFusionDataset(Dataset):
    """Fully in-memory dataset for tabular + image fusion training."""
...
class ImageMLP(nn.Module):
    """Late-fusion MLP over precomputed image embeddings and tabular features."""
    ...
    def __init__(
        ...
        prev_dim = image_dim + len(NUMERIC_COLS) + sum(embedding_dims)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train ImageMLP on precomputed CLIP embeddings")
    parser.add_argument(
        "--variant",
        required=True,
...
def _find_embedding_matrix(split: str, variant: str, requested_size: int) -> tuple[Path, int]:
    ...
    raise FileNotFoundError(
        f"Could not find an embedding matrix for split='{split}' variant='{variant}' image_size={requested_size}. "
        f"Looked for: {[path.name for path in candidates]}"
    )


def load_embedding_split(split: str, variant: str, requested_size: int) -> tuple[np.ndarray, np.ndarray, int]:
    matrix_path, actual_size = _find_embedding_matrix(split, variant, requested_size)
...
def load_split_bundle(split: str, variant: str, target_col: str, requested_size: int) -> tuple[SplitBundle, int]:
    ...
    image_embeddings, emb_ids, actual_size = load_embedding_split(split, variant, requested_size)
...
def evaluate_split(
    ...
    with torch.no_grad():
        for image_embeddings, categorical_features, numeric_features, _, y_raw, _ in loader:
            ...
            preds = model(image_embeddings, categorical_features, numeric_features).detach().cpu().numpy()
...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for image_embeddings, categorical_features, numeric_features, targets, _, sample_weight in train_loader:
            image_embeddings = image_embeddings.to(device, non_blocking=True)
            categorical_features = categorical_features.to(device, non_blocking=True)
            numeric_features = numeric_features.to(device, non_blocking=True)
            ...
            preds = model(image_embeddings, categorical_features, numeric_features)
        ...
        val_count = 0
        with torch.no_grad():
            for image_embeddings, categorical_features, numeric_features, targets, _, sample_weight in val_loader:
                image_embeddings = image_embeddings.to(device, non_blocking=True)
                categorical_features = categorical_features.to(device, non_blocking=True)
                numeric_features = numeric_features.to(device, non_blocking=True)
                targets = targets.to(device, non_blocking=True)
...
def main() -> None:
    ...
    print("=" * 80)
    print(f"ImageMLP | variant={variant} | target={target_col} | head={args.fusion_head} | image_size={args.image_size}")
    ...
    train_dataset = ImageFusionDataset(train_bundle)
    val_dataset = ImageFusionDataset(val_bundle)
    ...
    test_dataset = ImageFusionDataset(test_bundle)

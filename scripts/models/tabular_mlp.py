"""
TabularMLP training script for the Multi-Modal Airbnb Price Predictor.

Model registry coverage:
    #15  tabular_mlp.py  TabularMLP  normal_raw   shallow_64
    #16  tabular_mlp.py  TabularMLP  normal_bc    shallow_64
    #17  tabular_mlp.py  TabularMLP  cleaned_raw  shallow_64
    #18  tabular_mlp.py  TabularMLP  cleaned_bc   shallow_64
    #19  tabular_mlp.py  TabularMLP  normal_bc    deep_256   (head ablation)

This script is tabular-only (no text/image embeddings).
"""
...
class SplitBundle:
    """In-memory split bundle for one tabular split."""
...
class TabularDataset(Dataset):
    """Fully in-memory tabular dataset for PyTorch training."""
...
class TabularMLP(nn.Module):
    """Tabular-only MLP with embedding layers for categorical features."""
    ...
    def __init__(
        ...
        prev_dim = len(NUMERIC_COLS) + sum(embedding_dims)
    ...
    def forward(
        ...
        room_emb = self.room_type_embedding(cat[:, 0])
        neighbourhood_emb = self.neighbourhood_embedding(cat[:, 1])
        property_type_emb = self.property_type_embedding(cat[:, 2])
        instant_bookable_emb = self.instant_bookable_embedding(cat[:, 3])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train TabularMLP on tabular parquet splits")
    parser.add_argument(
        "--variant",
        required=True,
    ...
    parser.add_argument(
        ...
        default="shallow_64",
        help="Tabular MLP head label. Use deep_256 for run #19.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Optional dropout override; defaults to registry-aligned value.",
    )
    parser.add_argument(
        "--patience",
        type=int,
...
def load_split_bundle(split: str, variant: str, target_col: str) -> SplitBundle:
    ...
    return SplitBundle(
        listing_ids=df["listing_id"].to_numpy(dtype=str, copy=True),
...
def infer_cardinalities(*bundles: SplitBundle) -> dict[str, int]:
    """Infer safe categorical cardinalities across all loaded splits."""
...
def evaluate_split(
    ...
    with torch.no_grad():
        for categorical_features, numeric_features, _, y_raw, _ in loader:
            ...
            preds = model(categorical_features, numeric_features).detach().cpu().numpy()
...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for categorical_features, numeric_features, targets, _, sample_weight in train_loader:
            categorical_features = categorical_features.to(device, non_blocking=True)
            numeric_features = numeric_features.to(device, non_blocking=True)
            ...
            preds = model(categorical_features, numeric_features)
            loss = weighted_loss(preds, targets, sample_weight, target_transform)
            loss.backward()
            optimizer.step()

            batch_size = categorical_features.shape[0]
        ...
        val_count = 0
        with torch.no_grad():
            for categorical_features, numeric_features, targets, _, sample_weight in val_loader:
                categorical_features = categorical_features.to(device, non_blocking=True)
                numeric_features = numeric_features.to(device, non_blocking=True)
                targets = targets.to(device, non_blocking=True)
                sample_weight = sample_weight.to(device, non_blocking=True)
...
def main() -> None:
    ...
    print("=" * 80)
    print(f"TabularMLP | variant={variant} | target={target_col} | head={args.fusion_head}")
    ...
    print("Loading train/val tabular splits into RAM...")
    ...
    train_dataset = TabularDataset(train_bundle)
    val_dataset = TabularDataset(val_bundle)
    ...
    test_dataset = TabularDataset(test_bundle)

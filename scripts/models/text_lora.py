"""
TextLoRA — DistilBERT + tabular with LoRA fine-tuning.
Registry run #34: variant=normal_bc, head=deep_256, rank=16
...
Usage:
    python scripts/models/text_lora.py --variant normal_bc --lora-rank 16 --fusion-head deep_256 --run-name "ablation"
    python scripts/models/text_lora.py --variant normal_bc --lora-rank 16 --fusion-head deep_256 --smoke-test
"""
...
class TextLoRAModel(nn.Module):
    def __init__(self, tabular_dim: int, fusion_head_sizes: list[int]) -> None:
        super().__init__()
        ...
        self.fusion_head = _build_fusion_head(text_dim + TABULAR_EMBED_DIM, fusion_head_sizes)
    ...
    def forward(self, input_ids, attention_mask, tabular):
        ...
        return self.fusion_head(torch.cat([text_embed, tab_embed], dim=1))


def apply_lora(model: TextLoRAModel, lora_rank: int) -> int:
    cfg = LoraConfig(
        r=lora_rank,
...
def run_epoch(model, loader, optimizer, device, accum_steps, training: bool) -> float:
    ...
    with ctx:
        for step, batch in enumerate(loader):
            ...
            loss  = weighted_mse(preds, tgt, sw)
            if training:
...
@torch.no_grad()
def predict_raw(model, loader, device, price_transformer, target_col) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    all_preds, all_true = [], []
    for batch in loader:
        ids  = batch["input_ids"].to(device, non_blocking=True)
        mask = batch["attention_mask"].to(device, non_blocking=True)
        tab  = batch["tabular"].to(device, non_blocking=True)
        preds = model(ids, mask, tab).squeeze(1).cpu().numpy()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TextLoRA")
    ...
    print(f"\n{'='*70}")
    print(f"TextLoRA | variant={variant} | rank={args.lora_rank} | head={args.fusion_head}")
    ...
    tracker = ExperimentTracker(
        model_type="TextLoRA", modalities="tab+text", variant=variant,
        run_name=args.run_name, is_smoke_test=args.smoke_test,
        fusion_head=args.fusion_head, lora_applied_to="text", lora_rank=args.lora_rank,
        ...
        config={
            ...
            "max_text_len": MAX_TEXT_LEN, "lora_alpha": args.lora_rank * LORA_ALPHA_MULT,
            "lora_dropout": LORA_DROPOUT, "text_model": TEXT_MODEL_ID,
            "tabular_embed_dim": TABULAR_EMBED_DIM, "fusion_head_sizes": head_sizes,
    ...
    print("\nBuilding tokenizer and datasets (preloading into RAM)...")
    tokenizer = AutoTokenizer.from_pretrained(TEXT_MODEL_ID)
    train_ds  = TextLoRADataset(train_tab, train_raw, tokenizer, target_col)
    val_ds    = TextLoRADataset(val_tab,   val_raw,   tokenizer, target_col)
    test_ds   = TextLoRADataset(test_tab,  test_raw,  tokenizer, target_col)
    ...
    model            = TextLoRAModel(len(TABULAR_COLS), head_sizes).to(device)
